...
public:
    ...
    LinkListNode* searchBlock( const char* fileName, int block, unsigned int& g_Info ); //查询数据块是否在内存,查询到直接发送过去 ，并改变队列的位置
private:
    ...
    LinkListNode* getIoBlock( const char* fileName, int index, unsigned int& g_info ); //说明cache块数还没达到系统设定的要求

    LinkListNode* getIoBlock2( const char* fileName, int index, unsigned int& g_info ); // 系统的cache块数已经达到最大要求，要采用LRU算法替换
