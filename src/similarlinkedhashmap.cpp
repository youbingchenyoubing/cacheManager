...
LinkListNode* CacheManager::searchBlock( const char* fileName, int index, unsigned int& g_Info )
{
...
LinkListNode* CacheManager::getIoBlock( const char* fileName, int index, unsigned int& g_Info )
{
#ifdef __CACHE_DEBUG
    cout << "开启IO读取模式" << endl;
#endif
...
LinkListNode* CacheManager::getIoBlock2( const char* fileName, int index, unsigned int& g_Info )
{
...
bool IOManager::AIORead( const char* fileName, int index, shared_ptr <configureInfo> configureContent, unsigned  int& g_Info, LinkListNode* guardNode, bool flag )
{
    ...
    else
    {
        ...
        {
            cout << "WARNING!!!读出了" << event.res << "个字节，但是块的大小是：" << configureContent->blockSize << "字节，这个原因可能也是因为文件的末尾" << endl;
        }
#endif
        return true;
    }
