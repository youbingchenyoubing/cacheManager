...
class IOManager
{
  ...
  public:
      ...
      ~IOManager(){};
      bool AIORead(char * fileName,int index,shared_ptr <configureInfo> configureInfo,unsigned  int & g_Info,LinkListNode * guardNode);
  private:
  	   IOManager();
  	   //void IOMmap();
...
class CacheManager
{
  ...
  private:
    ...
    bool deleteMap(const string &fileName,int index);
