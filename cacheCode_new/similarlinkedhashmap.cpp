...
void * CacheManager::getIoBlock(char * fileName,int index,unsigned int & g_Info)
{
    ...
    if(!(ioInstance->AIORead(fileName,index,configureContent,g_Info,temp)))
    {
          mutex_Lock.unlock();
          m_maplocker.wrlock();
          delete temp;
...
void * CacheManager::getIoBlock2(char * fileName,int index,unsigned int & g_Info)
{
   ...
   string file = fileName;
    ...
    if(!(ioInstance->AIORead(fileName,index,configureContent,g_Info,guardNode)))
    {
          mutex_Lock.unlock();
    ...
    {
         m_maplocker.unlock();
         return mapManager[file][index];
    }
    deleteMap(file,index);
    addMap(file,index);
    m_maplocker.unlock();
    mutex_Lock.unlock();
    return linkListHead;
...
bool CacheManager::deleteMap(const  string &fileName,int index)
{
...
bool IOManager::AIORead(char *fileName,int index,shared_ptr <configureInfo> configureContent,unsigned  int & g_Info,LinkListNode * guardNode)
{
   int fd,ret = -1;
   ...
   char file[50];
   strcpy(file,configureContent->filePath);
